...
**VPS/CD:** image-based stack under `deploy/`. Naming SoT is `deploy/compose.yaml` `name: ${SHOPWARE_SHOP_ID}-${SHOPWARE_DEPLOY_ENV}` with host env files loaded. Raw operator Compose must use the same `--env-file` flags as fyrst-cli (not only `.env`). Omit `--env-file .env.prod` when that file is absent. `-p` is a matching pin, not the SoT.
