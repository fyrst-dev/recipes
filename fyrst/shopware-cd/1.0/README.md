...
Flex `env` may append a `###> fyrst/shopware-cd ###` block to shop-root `.env` (empty `SHOPWARE_SHOP_ID`, `SHOPWARE_DEPLOY_ENV=live`, `SHOPWARE_DATA_BASE=/var/lib/shopware/data`). It does not overwrite create’s whole `.env`. Then run `fyrst-cli shopware env init --shop-id <slug>` (see the copied `deploy/README.md`). That command sets `COMPOSE_PROJECT_NAME=shopware-<shop-id>` (example: shop id `acme` → `shopware-acme`; no env suffix) for local `shopware-cli project dev` / shop-root compose. It replaces create’s `COMPOSE_PROJECT_NAME=sw-shop-…`. VPS stacks keep project name `${SHOPWARE_SHOP_ID}-${SHOPWARE_DEPLOY_ENV}` (`acme-live`) via deploy Compose `name:`; fyrst-cli pins `-p` or unsets `COMPOSE_PROJECT_NAME` on VPS compose so `.env` does not override. Live and staging on one host stay isolated. It does not generate `APP_SECRET` (`shopware-cli project create` already writes it). Install fyrst-cli 0.1.0+ on each VPS.

VPS runtime DB + bind-mount pull (no object storage): `fyrst-cli shopware sync {capture|apply|pull}` — see `deploy/sync-runtime.md`. **Dump stays `shopware-cli project dump`.** **Sync is not a backup** — live backups are `fyrst-cli shopware backup {create|prune|recover}`. VPS Compose source of truth is `SHOPWARE_SHOP_ID` + `SHOPWARE_DEPLOY_ENV` (project name `acme-live`, uploads under `/var/lib/shopware/data/${SHOPWARE_SHOP_ID}/${SHOPWARE_DEPLOY_ENV}`). `COMPOSE_PROJECT_NAME=shopware-<shop-id>` is local/dev only. `SHOPWARE_DATA_ROOT` is an optional script override. Flex does not delete it on `composer require` (create’s `COMPOSE_PROJECT_NAME=sw-shop-…` stays until env init). Same-host tag-and-load: `PULL_POLICY=never` / `SKIP_PULL=1`.
