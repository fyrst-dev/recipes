...
#   pull → mysql/redis → setup profile → recreate web → extra profiles
...
usage() {
  cat <<'EOF'
Usage: deploy/vps-rollback.sh [--dry-run]

Re-deploy IMAGE with IMAGE_TAG from .previous-tag (pull + --no-build).
