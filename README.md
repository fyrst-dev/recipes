...
Flex copies CI, `deploy/` (CD Compose, edge), `.dockerignore`, and `.env.example` from the package `overlay/` into the shop root. The Flex `env` configurator may append a `###> fyrst/shopware-cd ###` block to `.env` (empty shop id, `SHOPWARE_DATA_BASE=/var/lib/shopware/data`). It does not put `SHOPWARE_DEPLOY_ENV` in that shared block (host `.env.local` owns deploy env), does not overwrite create’s whole `.env`, and does not put secrets in that block. Then run `fyrst-cli shopware env init --shop-id <slug>` (see `deploy/README.md`). That command always comments out create’s `COMPOSE_PROJECT_NAME` in committed `.env`. It does not write `COMPOSE_PROJECT_NAME` into `.env`. `--env` writes `SHOPWARE_DEPLOY_ENV` into gitignored `.env.local`, not the shared committed `.env`. VPS Compose project name is `${SHOPWARE_SHOP_ID}-${SHOPWARE_DEPLOY_ENV}` (`acme-live`) derived at runtime; fyrst-cli may pin `-p`. It does not generate `APP_SECRET` (`shopware-cli project create` already writes it). Install [fyrst-cli](https://github.com/fyrst-dev/cli) 0.1.0+ on each VPS. Commit the copied files; `vendor/` stays gitignored.
