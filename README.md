...
- **VPS/CD:** files under `deploy/` (Compose templates, edge). CI runs `fyrst-cli shopware deploy release`. Post-deploy probe: default is `APP_URL` (trailing slash stripped) + `/api/_info/health-check` (same path as the Compose `web` healthcheck). Override with `DEPLOY_HEALTH_URL` (full URL as-is, e.g. loopback). Live requires a resolvable probe URL unless `--allow-no-deploy-health` / `ALLOW_NO_DEPLOY_HEALTH=1`. Non-live: the probe is optional when no URL can be resolved. `SMOKE_URL` is a deprecated alias for `DEPLOY_HEALTH_URL` (not the operator SoT). Auto-rollback still uses `ROLLBACK_ON_SMOKE_FAIL`. Dump stays `shopware-cli project dump`. Create and image build stay out of this recipe.

Flex copies CI, `deploy/` (CD Compose, edge), `.dockerignore`, and `.env.example` from the package `overlay/` into the shop root. The Flex `env` configurator may append a `###> fyrst/shopware-cd ###` block to `.env` (empty shop id, `SHOPWARE_DATA_BASE=/var/lib/shopware/data`). It does not put `SHOPWARE_DEPLOY_ENV` in that shared block (host `.env.local` owns deploy env and `COMPOSE_PROJECT_NAME`), does not overwrite create’s whole `.env`, and does not put secrets in that block. Then run `fyrst-cli shopware env init --shop-id <slug>` (see `deploy/README.md`). That command always comments out create’s `COMPOSE_PROJECT_NAME` in committed `.env`. It does not write `COMPOSE_PROJECT_NAME` or a real `SHOPWARE_DEPLOY_ENV` into committed `.env`. It writes `SHOPWARE_DEPLOY_ENV` and `COMPOSE_PROJECT_NAME=<shop-id>-<env>` into gitignored `.env.local` (local `acme-dev`, VPS `acme-live` — same pattern, not the folder basename and not `shopware-<shop-id>`) and sets `compose.override.yaml` `name:` so `shopware-cli project dev` sees that project name. VPS Compose naming SoT is `deploy/compose.yaml` `name: ${SHOPWARE_SHOP_ID}-${SHOPWARE_DEPLOY_ENV}` with `--env-file .env` then `--env-file .env.local` then `--env-file .env.prod` (if present); fyrst-cli passes those same `--env-file` flags. Raw operator Compose must use those same `--env-file` flags (not only `.env`). It does not generate `APP_SECRET` (`shopware-cli project create` already writes it). Install [fyrst-cli](https://github.com/fyrst-dev/cli) 0.1.0+ on each VPS. Commit the copied files; `vendor/` stays gitignored.
